...
@Slf4j
public class TB07060ServiceImpl implements TB07060Service {
	...
	public IBIMS401BVO srchCrdlLdg(IBIMS401BVO input) {
		...
		if ( "1".equals(srchF) ) {
			...
			// 여신이자계산내역
			IBIMS406BDTO ibims406bdto = new IBIMS406BDTO();
			ibims406bdto.setPrdtCd(prdtCd);		// 종목코드
			ibims406bdto.setExcSn(excSn);		// 실행일련번호
			ibims406bdto.setTrSn(trSn);			// 거래일련번호
			List<IBIMS406BVO> intrTrDtls = ibims406bMapper.selectIntrTrDtls(ibims406bdto);
			resObj.setIntrTrDtls(intrTrDtls);
	
			// 딜수수료수납내역
			IBIMS420BDTO ibims420bdto = new IBIMS420BDTO();
			ibims420bdto.setPrdtCd(prdtCd);		// 종목코드
			ibims420bdto.setExcSn(excSn);		// 실행일련번호
			ibims420bdto.setTrSn(trSn);			// 거래일련번호
			List<IBIMS420BVO> feeRcivDtls = ibims420bMapper.selectFeeRcivDtls(ibims420bdto);
			resObj.setFeeRcivDtls(feeRcivDtls);
		}
	...
	public IBIMS410BVO inqTrDtls(IBIMS410BDTO input) {
		...
		List<IBIMS410BVO> dlTrList = ibims410bMapper.getDlTrList(input);
