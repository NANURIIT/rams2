...
function selectBox() {
    ...
    objSlc.X001.filter(it =>  ['4', '5', '6', '7'].includes(it.cdValue))
               .forEach(item => {
            ...
            if( data && data.batSchM.length > 0 ){
                ...
                batSchM.on('rowSelect', function(evt, ui){
                    ...
                    if ( al.length > 0 ) {
                         ...
                         *  이렇게 선택하셔야 합니다
                         */
                        $('input[name="TB10610S_exc_jobSts"][value="7"]').prop('checked', true);
...
function excBatch() {
    ...
    let jobSts_txt = $(`input[value=${jobStatus}][name="TB10610S_exc_jobSts"]:checked`).next('label').text()
