...
function selectBoxSet_TB07090S(){
    selectBox = getSelectBoxList('TB07090S', 'E020/D010/I027/F008/S001/R038', false);

    dprtList = selectBox.filter(function(item){                 //부서코드 list
		return item.cmnsGrpCd === 'D010';
	});

    crryCdList = selectBox.filter(function(item){               //통화코드 list
        return item.cmnsGrpCd === 'I027';
    });

    fndsDcdList = selectBox.filter(function(item){              //자금구분코드 list
        return item.cmnsGrpCd === 'F008';
    });

    scxDcdList = selectBox.filter(function(item){              //상환구분코드 list
        return item.cmnsGrpCd === 'S001';
    });

    rdptObjtDvsnCdList = selectBox.filter(function(item){      //상환대상구분코드 list
...
function TB07090S_addNewRow(colModelId) {
    ...
    if(colModelId == "TB07090S_colModel2"){             //입금증등록내역 행 추가
        ...
        if(!isEmpty(selected_rdmpPrarDtl)){
            ...
            if($("#" + colModelId).pqGrid('option', 'dataModel.data').length > 0){
                ...
                for(var i = 0; i < $("#" + colModelId).pqGrid('option', 'dataModel.data').length; i++){
                    var rowData = $("#" + colModelId).pqGrid("getRowData", {rowIndx: i});
                    ...
                    if(bfDealNo === selected_rdmpPrarDtl.dealNo && Number(bfRctmAmt) === Number(selected_rdmpPrarDtl.pmntPrarAmt)){
                ...
                if(!isExist){
                    ...
                    $("#" + colModelId).pqGrid("addRow", { rowData: newRow, checkEditable: false });
        ...
        if(isEmpty(selected_rdmpPrarDtl)){
            ...
            openPopup(option);
            return false;
        }else if(selected_rdmpPrarDtl.dealNo != selected_dptrRgstDtl.dealNo || selected_rdmpPrarDtl.pmntPrarAmt != selected_dptrRgstDtl.pmntPrarAmt){
            ...
            if($("#" + colModelId).pqGrid('option', 'dataModel.data').length > 0){
                ...
                for(var i = 0; i < $("#" + colModelId).pqGrid('option', 'dataModel.data').length; i++){
                    ...
                    if(bfDealNo === selected_dptrRgstDtl.dealNo && Number(bfRctmAmt) === Number(selected_dptrRgstDtl.rctmDt)){
...
function TB07090S_deleteRow(colModelId, yourFunction) {
    ...
    let getLength = colModelIdSelector(colModelId).pqGrid("instance").pdata.length;
    let colModel = colModelIdSelector(colModelId);

    if (TB07090S_rowData != TB07090S_dummyData && TB07090S_pqGridLength < getLength && !TB07090S_rowData.excSn) {
        colModel.pqGrid("deleteRow", { rowData: TB07090S_rowData, checkEditable: false });
        TB07090S_rowData = TB07090S_dummyData;
    } else if (TB07090S_rowData === TB07090S_dummyData && TB07090S_pqGridLength < getLength) {
        colModel.pqGrid("deleteRow", { rowData: TB07090S_rowData, checkEditable: false });
        TB07090S_rowData = TB07090S_dummyData;
    } else if (TB07090S_rowData === TB07090S_dummyData && TB07090S_pqGridLength === getLength) {
...
function inq(param){
    $.ajax({
        ...
        dataType: "json",
        beforeSend: function(){
			$("#TB07090S_colModel1").pqGrid("option", "strNoRows", "조회 중입니다...");
            $("#TB07090S_colModel2").pqGrid("option", "strNoRows", "조회 중입니다...");
            $("#TB07090S_colModel3").pqGrid("option", "strNoRows", "조회 중입니다...");
		},
        success: function(data) {
            ...
            if(rdmpPrarDtlsList.length < 1 && rctmDtlsList.length < 1 && dptrDtlsList.length < 1){
                var option = {}
                option.title = "Error";
                option.type = "error";
...
function rctmDtlsRgst(){
    ...
    var gridLgth = $("#TB07090S_colModel2").pqGrid('option', 'dataModel.data').length;
    var paramCheck = false;

    var paramList = [];

    for(var i = 0; i < gridLgth; i++){
        var rowData = $("#TB07090S_colModel2").pqGrid("getRowData", {rowIndx: i});
        ...
        if(isEmpty(rowData.rgstSeq)){
            ...
            var formattedAplcAmt =  aplcAmt.includes(',') ? aplcAmt.replaceAll(',', '') : aplcAmt;
...
function rctmDtlsMapping(){
    ...
    var gridLgth = $("#TB07090S_colModel3").pqGrid('option', 'dataModel.data').length;

    var paramList = [];

    for(var i = 0; i < gridLgth; i++){
        var rowData = $("#TB07090S_colModel3").pqGrid("getRowData", {rowIndx: i});
