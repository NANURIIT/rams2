...
$(document).ready(function () {
    $("input[id*='Amt'], input[id*='Blce'], input[id*='Rt'], input[id*='Mnum'], input[id*='Tmrd']").val('0');
...
function fnSelectBox() {
    selectBox = getSelectBoxList("TB07080",
        "/L001"
        + "/I017"
        + "/I013"
        + "/A007"
        + "/S003"
        , false);
    grdSelect.L001 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'L001'; });		//	RPTS_CTRT_TP_DETL_CD
    grdSelect.I017 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'I017'; });		//	DET_GRTE_FLFL_DCD
    grdSelect.I013 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'I013'; });
    grdSelect.A007 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'A007'; });
    grdSelect.S003 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'S003'; });
}
...
function TB07080S_addNewRow() {
    ...
    $("#TB07080S_colModel").pqGrid("addRow", { rowData: newRow, checkEditable: false });
}
...
function TB07080S_deleteRow() {
    ...
    if (TB07080S_rowData != dummyData && TB07080S_pqGridLength < getLength && !TB07080S_rowData.excSn) {
        $("#TB07080S_colModel").pqGrid("deleteRow", { rowData: TB07080S_rowData, checkEditable: false });
        TB07080S_rowData = dummyData;
    } else if (TB07080S_rowData === dummyData && TB07080S_pqGridLength < getLength) {
        $("#TB07080S_colModel").pqGrid("deleteRow", { rowData: TB07080S_rowData, checkEditable: false });
        TB07080S_rowData = dummyData;
    } else if (TB07080S_rowData === dummyData && TB07080S_pqGridLength === getLength) {
        Swal.fire({
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data) {
                ...
                $('#TB07080S_lastPrnaRdmpDt').val(getDateData(data.lastPrnaRdmpDt, "yyyymmdd", "-"))
                $('#TB07080S_lastPrnaRdmpDt_d').val(getDateData(data.lastPrnaRdmpDt, "yyyymmdd", "-"))
                $('#TB07080S_lastIntrClcDt').val(getDateData(data.lastIntrClcDt, "yyyymmdd", "-"))
                $('#TB07080S_lastIntrClcDt_d').val(getDateData(data.lastIntrClcDt, "yyyymmdd", "-"))
                $('#TB07080S_nxtRdmpPrarDt').val(getDateData(data.nxtRdmpPrarDt, "yyyymmdd", "-"))
                $('#TB07080S_nxtRdmpPrarDt_d').val(getDateData(data.nxtRdmpPrarDt, "yyyymmdd", "-"))
                $('#TB07080S_nxtIntrPymDt').val(getDateData(data.nxtIntrPymDt, "yyyymmdd", "-"))
                $('#TB07080S_nxtIntrPymDt_d').val(getDateData(data.nxtIntrPymDt, "yyyymmdd", "-"))
                ...
                $('#TB07080S_prnaOvduDt').val(getDateData(data.prnaOvduDt, "yyyymmdd", "-"))
                $('#TB07080S_prnaOvduDt_d').val(getDateData(data.prnaOvduDt, "yyyymmdd", "-"))
                $('#TB07080S_intrOvduDt').val(getDateData(data.intrOvduDt, "yyyymmdd", "-"))
                $('#TB07080S_intrOvduDt_d').val(getDateData(data.intrOvduDt, "yyyymmdd", "-"))
...
function updateExcData() {
    ...
    paramData = {
        ...
        , lastPrnaRdmpDt: getDateData($('#TB07080S_lastPrnaRdmpDt').val(), "yyyymmdd", "")
        , lastIntrClcDt: getDateData($('#TB07080S_lastIntrClcDt').val(), "yyyymmdd", "")
        , nxtRdmpPrarDt: getDateData($('#TB07080S_nxtRdmpPrarDt').val(), "yyyymmdd", "")
        , nxtIntrPymDt: getDateData($('#TB07080S_nxtIntrPymDt').val(), "yyyymmdd", "")
