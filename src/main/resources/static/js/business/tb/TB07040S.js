const TB07040Sjs = (function () {
        ...
        if (data.length > 0) {
          ...
          data.forEach(function (item) {
            ...
            var newRow = {
              ...
              prfdCorpIntx: isEmpty(item.prfdCorpIntx)
                ? ""
                : addComma(item.prfdCorpIntx),
              ...
              stdrExrt: isEmpty(item.stdrExrt)
                ? "0.00"
                : addComma(item.stdrExrt),
              excSn: isEmpty(item.excSn) ? "" : addComma(item.excSn),
              wholIssuShqt: isEmpty(item.wholIssuShqt)
                ? "0"
                : addComma(item.wholIssuShqt),
              hldgShqt: isEmpty(item.hldgShqt) ? "0" : addComma(item.hldgShqt),
              qotaRt: isEmpty(item.qotaRt)
                ? "0.00"
                : addComma(Number(item.qotaRt).toFixed(8)),
              evlPflsAmt: isEmpty(item.evlPflsAmt)
                ? "0"
                : addComma(item.evlPflsAmt),
              tradPflsAmt: isEmpty(item.tradPflsAmt)
                ? "0"
                : addComma(item.tradPflsAmt),
              eprzCrdlCtrcAmt: isEmpty(item.eprzCrdlCtrcAmt)
                ? ""
                : addComma(item.eprzCrdlCtrcAmt),
              krwTrslExcBlce: isEmpty(item.krwTrslExcBlce)
                ? "0"
                : addComma(item.krwTrslExcBlce),
              krwTrslExcAmt: isEmpty(item.krwTrslExcAmt)
                ? "0"
                : addComma(item.krwTrslExcAmt),
  ...
  function makeParam() {
    ...
    function bessniseFunction(trSn, inputDcd) {
      ...
      var fee =
        Number($("#TB07040S_trslAmt").val().replaceAll(",", "")) *
        g_feeRt *
        g_isttRt; // 수수료
      fee = (inputDcd == "1" ? 1 : -1) * fee;
      var prfdCorpIntx = Number(
        $("#TB07040S_prfdCorpIntx").val().replaceAll(",", "")
      ); // 선급법인세
      prfdCorpIntx = (inputDcd == "1" ? 1 : -1) * prfdCorpIntx;
      var lprfdCorpIntx = Number(
        $("#TB07040S_prfdCorpIntx").val().replaceAll(",", "")
      ); // 선급법인세
      ...
      var acbkAmt =
        (inputDcd == "1" ? 1 : -1) *
        ((lTrAmt + lTrtx + lFee + Number(lprfdCorpIntx)) * -1); // 장부금액
      var dealNo = 0; // 딜번호
      var mtrDcd; // 안건구분코드
      var jdgmDcd; // 심사구분코드
      ...
      if (inputDcd == "1") {
        ...
        if (Number(g_sllPosShqt) - Number(paramData.trQnt) < 0) {
          ...
          Swal.fire({
            ...
            title: "Error!",
            html:
              "거래좌수가 보유좌수를 초과하였습니다.<br> (초과좌수:" +
              addComma(Math.abs(overTrQnt)) +
              ")",
            confirmButtonText: "확인",
          });
          //openPopup(option);
  ...
  function calcTrAmt() {
    ...
    if (ibPrdtTrDcd === "82") {
      ...
      if (
        isNotEmpty(
          $("#TB07040S_trQnt").val() && isNotEmpty($("#TB07040S_trUnpr").val())
        )
      ) {
        $("#TB07040S_trAmt").val(
          addComma(
      ...
      if (
        isNotEmpty($("#TB07040S_trQnt").val()) &&
        Number($("#TB07040S_wholIssuShqt").val()) !== 0
      ) {
        $("#TB07040S_qotaRt").val(
          (
  ...
  function calcTrslAmt() {
    if (
      isNotEmpty(
        $("#TB07040S_trAmt").val() && isNotEmpty($("#TB07040S_trdeExrt").val())
      )
    ) {
      $("#TB07040S_trslAmt").val(
        addComma(
          Number($("#TB07040S_trAmt").val().replaceAll(",", "")) *
            Number($("#TB07040S_trdeExrt").val().replaceAll(",", ""))
        )
      );
      // 거래세계산
      $("#TB07040S_trtx").val(
        addComma(
          Number($("#TB07040S_trslAmt").val().replaceAll(",", "")) * g_trtxRt
        )
      );
    }
  }
