...
$(document).ready(function() {
	 ...
	 */
	getSelect = getSelectBoxList('TB07010S', 'E010/S003/E020/E011/F008/I017/F006/R023/I027', false);
...
function pqGrid() {
	...
	let colFeeRciv = [
		...
		{ 
			...
			halign   :  "center",
			colModel : [
						{ title : "대상금액", dataType : "integer",  dataIndx : "feeStdrAmt", halign : "center", align : "right",  format : '#,###.00' },
						{ title : "대상내용", dataType : "string", dataIndx : "feeTrgtCtns", halign : "center", align : "left" },
						{ title : "율(%)", dataType : "string",  dataIndx : "feeRt", halign : "center", align : "right"},
						{ title : "금액", dataType : "integer", dataIndx : "feeAmt", halign : "center", align : "right", format : '#,###.00' }
					   ]
		},
		{ 	
			title    : "처리완료여부", 
		...
		$.ajax({
			...
			success: function (data) {
				...
				if( data ) {
					...
					$('#TB07010S_ptxtTrOthrDscmNo').val(isEmpty(data.ptxtTrOthrDscmNo)?'':checkBrnAcno(data.ptxtTrOthrDscmNo)); // 거래상대방
					...
					$('#TB07010S_regDt').val(isEmpty(data.eprzCrdlApvlDt)?'':formatDate(data.eprzCrdlApvlDt));
					$('#TB07010S_ctrcDt').val(isEmpty(data.ctrcDt)?'':formatDate(data.ctrcDt));
					$('#TB07010S_ctrcExpDt').val(isEmpty(data.ctrcExpDt)?'':formatDate(data.ctrcExpDt));
					$('#TB07010S_I027').val(crryCd);						// 통화코드
					$('#TB07010S_eprzCrdlApvlAmt').val(isEmpty(data.eprzCrdlApvlAmt) ? '' : addComma(data.eprzCrdlApvlAmt)); // 승인금액
					...
					if ( crryCd === "KRW" ) {
						$('#TB07010S_contractAmt').val(isEmpty(contractAmt) ? '' : addComma(Math.round(contractAmt))); 	// 약정금액
						$('#TB07010S_loanAmt').val(isEmpty(loanAmt) ? '' : addComma(Math.round(loanAmt))); 			   	// 대출금액
						$('#TB07010S_dealExcBlce').val(isEmpty(dealExcBlce) ? '' : addComma(Math.round(dealExcBlce)));	// 대출잔액
					} else {
						$('#TB07010S_contractAmt').val(isEmpty(contractAmt) ? '' : addComma(contractAmt.toFixed(2))); 	// 약정금액
						$('#TB07010S_loanAmt').val(isEmpty(loanAmt) ? '' : addComma(loanAmt.toFixed(2))); 			   	// 대출금액
						$('#TB07010S_dealExcBlce').val(isEmpty(dealExcBlce) ? '' : addComma(dealExcBlce.toFixed(2)));	// 대출잔액
					}
					...
					} else if ( eprzCrdlIndvLmtDcd === '2' ) { // 한도구분 == 2:한도
	...
	if ( validation().isValid ) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				$('#TB07010S_prcsIntrAmt').val(commaNull(Math.round(data.rdmpPrarIntr)))
...
function save() {
	...
	if( validation().isValid ) {
		...
		/* 기업여신정보 */
		let ptxtTrOthrDscmNo = $('#TB07010S_ptxtTrOthrDscmNo').val().replaceAll('-', '');
...
function ctrlComp(f, obj={}) {
	switch (f) {
		...
		case 'btnSave':
			...
			// 버튼 컨트롤
			if ( obj.eprzCrdlIndvLmtDcd === '1' && obj.loanablAmt <= 0 ) { // 개별(분할실행) (1)
				// console.log('개별(분할실행) (1)');
				$('#btnSave').prop("disabled", true);
				obj={}
				// console.log('개별(분할실행) (1) ::: obj', obj);
				
			} else if ( obj.eprzCrdlIndvLmtDcd === '2' && obj.eprzCrdlApvlAmt <= obj.dealExcBlce) { // 한도 (2)
				// console.log('한도 (2)');
				// console.log('타니');
				
				$('#btnSave').prop("disabled", true); 
				obj={}
			} else if ( obj.eprzCrdlIndvLmtDcd === '3' && obj.loanablAmt <= 0 ) { // 개별(일괄실행) (3)
...
function validation() {
	...
	if ( fValid === '2' ) {
		...
		if ( numContractExprDt < numExpDt ) {
			sf(1, 'warning', `[기업여신정보]약정만기일보다<br>[실행정보]만기일자가 이후일 수 없습니다.`)
			return { isValid : false };
		}
