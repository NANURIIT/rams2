...
function pqGrid() {
	...
	let col_trDtls = [
		...
		{ 	
            ...
            render   : function(ui) {
                let fSel = grdSelect.E026.find(({ cdValue }) => cdValue == ui.cellData );
		...
		{ 	
            ...
            render   : function(ui) {
                let fSel = grdSelect.E025.find(({ cdValue }) => cdValue == ui.cellData );
                return fSel ? fSel.cdName : ui.cellData;
            },
		},
        { 	
	...
	let col_intrTrDtls = [
		...
		{ 	
            ...
            render   : function(ui) {
                let fSel = grdSelect.P013.find(({ cdValue }) => cdValue == ui.cellData );
                return fSel ? fSel.cdName : ui.cellData;
            },
		},
        { 	
    ...
    let col_feeRcivDtls = [
		...
		{ 	
            ...
            render   : function(ui) {
                let fSel = grdSelect.E008.find(({ cdValue }) => cdValue == ui.cellData );
                return fSel ? fSel.cdName : ui.cellData;
            },
		},
        { 	
        ...
        { 	
            ...
            render   : function(ui) {
                let fSel = grdSelect.T006.find(({ cdValue }) => cdValue == ui.cellData );
                return fSel ? fSel.cdName : ui.cellData;
            },
		},
        { 	
    ...
    let col_excIntrtInf = [
        ...
        },
        {
            ...
            render   : function(ui) {
                let fSel = grdSelect.S003.find(({ cdValue }) => cdValue == ui.cellData );
                return fSel ? fSel.cdName : ui.cellData;
            },
        },
        {
        ...
        },
        {
            ...
            render   : function(ui) {
                let fSel = grdSelect.I013.find(({ cdValue }) => cdValue == ui.cellData );
                return fSel ? fSel.cdName : ui.cellData;
            },
        },
        {
        ...
        },
        {
            ...
            render   : function(ui) {
                let fSel = grdSelect.A007.find(({ cdValue }) => cdValue == ui.cellData );
                return fSel ? fSel.cdName : ui.cellData;
            },
        }, 
        {
        ...
        success: function(data) {
			...
			if( data && data.length > 0 ){
				...
				data.forEach( item => {
					...
					html += '<option value="' + item.EXC_SN + '">' + item.EXC_SN + '</option>';
				});
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if ( data ) {
                    ...
                    /* ROW1 */
                    $('#TB07060S_ptxtTrOthrDscmNo').val(checkBrnAcno(data.ptxtTrOthrDscmNo));   // 거래상대방 사업자등록번호
                    ...
                    $(`input[name="TB07060S_intrHdwtClcYn"][value="${data.intrHdwtClcYn}"]`).prop('checked', true); // 수납완료여부
                    /* ROW5 */
                    ...
                    $('#TB07060S_apvlAmt').val(commaNull(data.eprzCrdlApvlAmt.toFixed(2)));          // 승인금액
                    ...
                    if ( data.ibims402BDTO ) {
                        ...
                        $('#TB07060S_L001_2').val(data.ibims402BDTO.ldgSttsCd ? data.ibims402BDTO.ldgSttsCd : ''); // 실행상태코드
                        ...
                        $('#TB07060S_dealExcAmt').val(commaNull(data.ibims402BDTO.dealExcAmt));                    // 실행금액
                        //console.log("data.ibims402BDTO.dealExcBlce:::",data.ibims402BDTO.dealExcBlce);
                        $('#TB07060S_dealExcBlce_2').val(commaNull(data.ibims402BDTO.dealExcBlce));                // 실행잔액
                        $('#TB07060S_prnaDfrPrdMnum').val(data.ibims402BDTO.prnaDfrPrdMnum);                       // 거치기간개월수
                        $('#TB07060S_dfrExpMnum').val(data.ibims402BDTO.dfrExpMnum);                               // 거치만기개월수
                        $('#TB07060S_lastIntrClcDt').val(dateNull(data.ibims402BDTO.lastIntrClcDt));               // 최종이자계산일자
                        $('#TB07060S_nxtRdmpPrarDt').val(dateNull(data.ibims402BDTO.nxtRdmpPrarDt));               // 다음원금납일일자 ? 다음상환예정일자
                        $('#TB07060S_nxtIntrPymDt').val(dateNull(data.ibims402BDTO.nxtIntrPymDt));                 // 다음이자납입일자
                        $('#TB07060S_prnaRdmpFrqcMnum').val(data.ibims402BDTO.prnaRdmpFrqcMnum);                   // 원금상환주기
                        $('#TB07060S_intrRdmpFrqcMnum').val(data.ibims402BDTO.intrRdmpFrqcMnum);                   // 이자상환주기
                        $('#TB07060S_I017').val(data.ibims402BDTO.intrPymDtCd);                                    // 상환지정일
                        $('#TB07060S_prnaOvduDt').val(dateNull(data.ibims402BDTO.prnaOvduDt));                     // 원금연체시작일자
                        $('#TB07060S_intrOvduDt').val(dateNull(data.ibims402BDTO.intrOvduDt));                     // 이자연체시작일자
                        ...
                        $('#TB07060S_dealIstmBlce').val(data.ibims402BDTO.dealIstmBlce ? comma(data.ibims402BDTO.dealIstmBlce) : data.ibims402BDTO.dealIstmBlce = 0);         // 할부잔액
                        $('#TB07060S_dealEqlRdmpAmt').val(data.ibims402BDTO.dealEqlRdmpAmt ? comma(data.ibims402BDTO.dealEqlRdmpAmt) : data.ibims402BDTO.dealEqlRdmpAmt = 0); // 균등상환금액
                        $('#TB07060S_istmDtmRdmpAmt').val(data.ibims402BDTO.istmDtmRdmpAmt ? comma(data.ibims402BDTO.istmDtmRdmpAmt) : data.ibims402BDTO.istmDtmRdmpAmt = 0); // 할부일시상환금
                        $('#TB07060S_rcvbIntrAmt').val(commaNull(data.ibims402BDTO.rcvbIntrAmt));                  // 미수이자
                        $('#TB07060S_lastPrnaRdmpDt').val(dateNull(data.ibims402BDTO.lastPrnaRdmpDt));             // 최종원금상환일자
                        $('#TB07060S_krwTrslRt').val(data.ibims402BDTO.krwTrslRt);                                 // 적용환율
                        $('#TB07060S_krwTrslExcAmt').val(commaNull(data.ibims402BDTO.krwTrslExcAmt));              // 원화환산금액
                    } 
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if ( data ) {
                    ...
                    $('#TB07060S_dealIstmBlce').val(data.dealIstmBlce ? comma(data.dealIstmBlce) : data.dealIstmBlce = 0);         // 할부잔액
                    $('#TB07060S_dealEqlRdmpAmt').val(data.dealEqlRdmpAmt ? comma(data.dealEqlRdmpAmt) : data.dealEqlRdmpAmt = 0); // 균등상환금액
                    $('#TB07060S_istmDtmRdmpAmt').val(data.istmDtmRdmpAmt ? comma(data.istmDtmRdmpAmt) : data.istmDtmRdmpAmt = 0); // 할부일시상환금
...
// ajax전
function bfReset() {
	$('input[id^="TB07060S"]').filter(function() {
        const id = $(this).attr('id');
        return id !== 'TB07060S_prdtCd' && id !== 'TB07060S_prdtNm' && id !== 'TB07060S_excSn';
    }).each(function() {
        const $this = $(this);
        //console.log($this);
        $this.val(initialValues[$this.attr('id')]);
