	...
	if( validation().isValid ) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				$('#TB07030S_crdtGrntOvduIntAmt').val(comma(data.totalDTO.totalOvduIntr)) // 연체이자합계
...
function calculator(f, p={}) {
	...
	let crdtGrntOvduIntAmt = Number(uncomma($('#TB07030S_crdtGrntOvduIntAmt').val()));	// 연체이자
	...
	switch (f) {
		case "rdmpPrnaSmmAmt": // [상환대상총금액] = 연체이자 + 정상이자 + 상환대상원금 + 중도상환원금 + 중도상환수수료 - 미수이자 - 면제금액
			...
			tot = rdmpTrgtPrna + nrmlIntrAmt + crdtGrntOvduIntAmt +  dealMrdpPrca + mrdpFeeAmt - rcvbIntrSmmAmt - exmptSmmAmt ;
		...
		case "paiTypCd":	// 이자유형코드별 합계 : 2.정상이자, 4.연체이자
			...
			for (let i = 0; i < p.length; i++) {
				...
				if ( paiTypCd === '4' || paiTypCd === '5' ) {	// 4: 납부이자연체금액 
					totCgoia += amt
				};
			};
		...
		case "aplcExchR": // 적용환율
			...
			if ( isNaN(aplcExchR) ) {
				...
				$('#TB07030S_KRW_rdmpTrgtPrna').val(comma(Math.round(obj._rdmpTrgtPrna))); // 상환대상원금
				$('#TB07030S_KRW_nrmlIntrAmt').val(comma(Math.round(obj._nrmlIntrAmt))); // 정상이자
				$('#TB07030S_KRW_crdtGrntOvduIntAmt').val(comma(Math.round(obj._crdtGrntOvduIntAmt))); // 연체이자
				$('#TB07030S_KRW_dealMrdpPrca').val(comma(Math.round(obj._dealMrdpPrca))); // 중도상환원금
				$('#TB07030S_KRW_mrdpFeeAmt').val(comma(Math.round(obj._mrdpFeeAmt))); // 중도상환수수료
				$('#TB07030S_KRW_exmptSmmAmt').val(comma(Math.round(obj._exmptSmmAmt))); // 면제금액합계
				$('#TB07030S_KRW_rdmpPrnaSmmAmt').val(comma(Math.round(obj._rdmpPrnaSmmAmt))); // 상환대상총금액
...
function bfReset() {
	...
	$('input[id^="TB07030S"]').not(`${excludeIds}`).each(function() {
