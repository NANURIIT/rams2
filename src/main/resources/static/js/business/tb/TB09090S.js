...
function selectColModel(modelNum) {
	...
	switch (modelNum) {
		case ("01"): return colCpc1
		case ("02"): return colCpc2
		case ("03"): return colCpc3
	}
...
function insertCpc1Data() {
	...
	if (mode = "01") {
		url = "insertIBIMS701B"
		cpcList = $("#colCpc1").pqGrid('instance').getData();
		...
		url = "insertIBIMS702B"
		cpcList = $("#colCpc2").pqGrid('instance').getData();
		for (var i = 0; i < cpcList.length; i++) {
			if (!cpcList[i].jobChrrNm) {
				cpcList[i].jobChrrNm = $('#userEmpNm').val()
			}
		...
		url = "insertIBIMS703B"
		cpcList = $("#colCpc3").pqGrid('instance').getData();
		for (var i = 0; i < cpcList.length; i++) {
			if (!cpcList[i].jobChrrNm) {
				cpcList[i].jobChrrNm = $('#userEmpNm').val()
			}
...
$("#upload-file-input-TB09090S").change(function (event) {
	// $("#colCpc1").pqGrid("deleteRow", { rowIndx: arrRowIndxs, track: false });
	readExcel();
...
function addRows_cpc1Grid(rows) {
	rows.forEach(function (row) {
		...
		$("#colCpc1").pqGrid("addRow", { rowData: newRow, checkEditable: false });
	});
	$("#colCpc1").pqGrid("refreshDataAndView");
}
...
function addRows_cpc2Grid(rows) {
	rows.forEach(function (row) {
		...
		$("#colCpc2").pqGrid("addRow", { rowData: newRow, checkEditable: false });
	});
	$("#colCpc2").pqGrid("refreshDataAndView");
}
...
function addRows_cpc3Grid(rows) {
	rows.forEach(function (row) {
		...
		$("#colCpc3").pqGrid("addRow", { rowData: newRow, checkEditable: false });
	});
	$("#colCpc3").pqGrid("refreshDataAndView");
}
