...
function selBox() {
    selectBox = getSelectBoxList("TB08040S", "F004"   // 수수료종류코드 FEE_KND_CD
...
// feeRt                   	     /* 수수료율      */
	...
	let col_feeSch = [
		...
		{ 	
            ...
            render   : function(ui) {
                let fSel = grdSelect.F004.find(({ cdValue }) => cdValue == ui.cellData );
        ...
        { 	
            ...
            render   : function(ui) {
                let fSel = grdSelect.E027.find(({ cdValue }) => cdValue == ui.cellData );
                return fSel ? fSel.cdName : ui.cellData;
            },
            editable   : true,
        },
		...
		{ 	
			...
			filter     : { crules: [{ condition: 'range' }] }, 
            colModel   : [
                            { title : "대상금액", dataType : "integer",  dataIndx : "feeStdrAmt", align : "right", halign : "center", align: "right", width : "10%", format : '#,###.00', editable : true, },
                            { title : "대상내용(계산식)", dataType : "string", dataIndx : "feeTrgtCtns", halign: "center", align : "left",  width : "10%", editable : true},
                            { title : "율(%)", dataType : "string",  dataIndx : "feeRt", halign: "center", align : "right",  width : "10%", editable : true,
                                editor: {
                                    type: 'textbox',
                                    init: function(ui) {
                            ...
                            { title : "수수료금액", dataType : "integer",  dataIndx : "feeAmt", halign: "center", align : "right", width : "10%", format : '#,###.00'},
                         ],
                         editable : true,
		},
        { 	
        ...
        { 	
            ...
            render   : function(ui) {
                let fSel = grdSelect.F001.find(({ cdValue }) => cdValue == ui.cellData );
                return fSel ? fSel.cdName : ui.cellData;
            },
            editable   : true,
		},
...
function validation(arr=[]) {
    ...
    if ( fValid === 1 ) {   // 저장
        for (let i = 0; i < arr.length; i++) {
            ...
            if ( ele.feeRcogDcd === '2' ) {
                if ( !ele.fnnrPrlnRto ) { // 이연이율
                    sf(2, 'warning', `[이연비율]`)
                    return { isValid : false };
                }
                if ( !ele.prlnFee ) { // 이연수수료
                    sf(2, 'warning', `[이연수수료]`)
                    return { isValid : false };
                }  
                if ( !ele.fnnrRcogStrtDt ) { // 인식시작일자
                    sf(2, 'warning', `[인식시작일자]`)
                    return { isValid : false };
                }
                if ( !ele.fnnrRcogEndDt ) { // 인식종료일자
                    sf(2, 'warning', `[인식종료일자]`)
                    return { isValid : false };
                }
            }
    ...
    switch (p) {
        case "p":
            if ( validation().isValid ) {
                ...
                //console.log(gLen);
                feeSch.addRow( { rowIndx: feeSchLen + 1, rowData: newRow, checkEditable: false } );
            };
            break;
        case "m":
            ...
            if ( validation().isValid ) {
                ...
                filteredIndexes.sort((a, b) => b - a).forEach(index => {
...
function chkGrd(p) {
	...
	for (let i = 0; i < data.length; i++) {
		...
		if ( chkData.chk && !addedIndx.has(i) 
     ...
     saveGrid = saveGrid.filter(item => {
        const isPresentInData = data.some(dataItem => dataItem === item && dataItem.chk)
