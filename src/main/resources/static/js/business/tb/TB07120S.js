...
function fnSelectBox() {
	selectBox = getSelectBoxList("TB07120S",
		"D016"              //  결재단계구분코드
		// + "/R031"           //  입출금구분코드
		+ "/I027"           //  통화구분코드
		// + "/D015"           //  업무구분코드
    ...
    TB07120S_grdSelect.D016 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'D016'; });
	TB07120S_grdSelect.I027 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'I027'; });
	// TB07120S_grdSelect.D015 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'D015'; });
	TB07120S_grdSelect.D006 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'D006'; });
	TB07120S_grdSelect.F008 = selectBox.filter(function (item) { return item.cmnsGrpCd === 'F008'; });
};
...
function createOption() {
    ...
    TB07120S_grdSelect.D016.forEach(item => { D016html += `<option value="${item.cdValue}">${item.cdName}</option>` });
    TB07120S_grdSelect.I027.forEach(item => { I027html += `<option value="${item.cdValue}">${item.cdName}</option>` });
    // TB07120S_grdSelect.D015.forEach(item => { D015html += `<option value="${item.cdValue}">${item.cdName}</option>` });
    TB07120S_grdSelect.D006.forEach(item => { D006html += `<option value="${item.cdValue}">${item.cdName}</option>` });
    TB07120S_grdSelect.F008.forEach(item => { F008html += `<option value="${item.cdValue}">${item.cdName}</option>` });
    ...
    $('#TB07120S_consDecdStatCd, #ibims452 #TB07120S_consDecdStatCd').append(D006html);
...
function decdStatChk(){
    ...
    // 승인요청버튼만 활성화
    if(consDecdStatCd === '0' || consDecdStatCd === '3' || consDecdStatCd === '4'){
...
function updateFndsCnstDecd(consDecdStatBtnNo){
    ...
    $.ajax({
        ...
        if (result <= 0) {
            Swal.fire({
                icon: result == -1 ? 'warning' : result == 0 ? 'warning' : result == -2 ? 'error' : 'info',
                text: swalText + ' 실패!'
            })
        }else {
            Swal.fire({
