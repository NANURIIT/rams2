...
$("#addFile").click(function(){
	...
	// 딜 정보 validation check
	if( isEmpty($('#fileSq').val())){
		Swal.fire({
			icon              : 'error'
			, title             : "Error!"
			, text              : "딜 정보 저장 후 파일추가가 가능합니다."
...
var callFileUpload = function (mode) {
	...
	if(isEmpty($('#fileSq').val())){
		if($("input[id*='ibDealNo']").length > 0){
			var ibDealNo = $('#fileIbDealNo').val($("input[id*='ibDealNo']").val());
		}
		if($("input[id*='riskInspctCcd']").length > 0){
			var riskInspctCcd = $('#fileRiskInspctCcd').val($("input[id*='riskInspctCcd']").val());
		}
		if($("input[id*='lstCCaseCcd']").length > 0){
			var lstCCaseCcd = $('#fileLstCCaseCcd').val($("input[id*='lstCCaseCcd']").val());
		}	
	}
	var msg = "";
	var nullCount = 0;
